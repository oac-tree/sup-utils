...
public:
  ...
  // multiple flag (boolean) accessors: return true if at least one of the flag appeared, otherwise
  // false.
                          ...
                          int mode /*= PREFER_FLAG_FOR_UNREG_OPTION*/)
{
  ...
  for (auto i = 0u; i < args_.size(); ++i)
  {
    ...
    // if 'name' is a pre-registered option, then the next arg cannot be a free parameter to it is
    // skipped otherwise we have 2 modes: PREFER_FLAG_FOR_UNREG_OPTION: a non-registered 'name' is
    // determined a flag.
    //                               The following value (the next arg) will be a free parameter.
    //
    // PREFER_PARAM_FOR_UNREG_OPTION: a non-registered 'name' is determined a parameter, the next
    // arg
